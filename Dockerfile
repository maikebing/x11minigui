...
RUN echo "deb http://mirrors.tuna.tsinghua.edu.cn/ubuntu/ xenial main restricted universe multiverse " > /etc/apt/sources.list && \
    ...
    apt-get -y  - -q update &&  apt-get install  -y   -q   ca-certificates x11-xserver-utils  dbus-x11  libcanberra-gtk3-module    libpci3 libgl1 && \
...
RUN   git clone https://gitlab.fmsoft.cn/VincentWei/build-minigui-5.0 && cd build-minigui-5.0/ && cp config.sh myconfig.sh && \
     ./fetch-all.sh  &&   ./build-deps.sh &&  sed -i -e 's/pci_system_init/_pci_system_init/g'  minigui/configure.ac && \
     && ./build-minigui.sh ths -splash -videopcxvfb -videodrm -savescreen -aboutdlg -pngsupport -webpsupport -pixman &&  \
